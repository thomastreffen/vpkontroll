...
- Returner kun tool-kallet, ingen annen tekst`;
    ...
    if (!response.ok) {
      ...
      if (response.status === 402) {
        return new Response(JSON.stringify({ error: "AI-kreditter brukt opp. Fyll på i innstillinger." }), {
          status: 402,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      return new Response(JSON.stringify({ error: "Kunne ikke generere forslag" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
